...
const OVERLAY_ID = '__xhs_cli_agent_operating__';
const STYLE_ID = '__xhs_cli_agent_operating_style__';

const DEFAULT_AGENT_BRAND = 'xhs-cli';

function resolveAgentBrand(): string {
  const raw = process.env.XHS_CLI_AGENT_BRAND;
...
/** 设为 `1` / `true` 时不注入操作蒙层。 */
export const SKIP_AGENT_OPERATING_OVERLAY =
  process.env.XHS_CLI_NO_AGENT_OVERLAY === '1' ||
  process.env.XHS_CLI_NO_AGENT_OVERLAY === 'true';

/**
 * 全屏半透明彩虹蒙层（呼吸式明暗脉冲），文案视口居中；`pointer-events: none` 不拦截页面操作。
 * 品牌标识取自 `XHS_CLI_AGENT_BRAND`（默认 `xhs-cli`）。
 */
export async function showAgentOperatingIndicator(page: Page): Promise<void> {
  const brand = resolveAgentBrand();
  await page.evaluate(`(() => {
    const overlay = ${JSON.stringify(OVERLAY_ID)};
    ...
    styleEl.textContent = \`
      ...
      #\${overlay} {
        ...
        animation: xhs-cli-breathe 2.6s ease-in-out infinite;
        display: flex;
        align-items: center;
        justify-content: center;
        padding: 1rem;
      }
      #\${overlay} .xhs-cli-agent-label {
    ...
    label.className = 'xhs-cli-agent-label';
...
/** 移除 {@link showAgentOperatingIndicator} 注入的样式与蒙层（幂等）。 */
